...
SDL_WriteToDataQueue(SDL_DataQueue *queue, const void *_data, const size_t _len)
{
    ...
    if (!queue) {
        SDL_SetError("SDL_WriteToDataQueue queue is NULL");
        return -1;
    }
...
SDL_ReserveSpaceInDataQueue(SDL_DataQueue *queue, const size_t len)
{
    ...
    if (!queue) {
		SDL_SetError("SDL_ReserveSpaceInDataQueue queue is NULL");
        return NULL;
    } else if (len == 0) {
		SDL_SetError("SDL_ReserveSpaceInDataQueue len is 0");
        return NULL;
